...
if (SPAWN_BACKEND) {
  backendProcess = spawn(process.execPath, [path.join(__dirname, 'browser-backend.js')], {
    cwd: serverDir,
    env: { ...process.env, PORT: String(INTERNAL_PORT), API_KEY: INTERNAL_API_KEY },
    stdio: 'inherit'
  });
  backendProcess.on('exit', (code, signal) => {
    if (signal) console.error(`Browser backend exited from signal ${signal}`);
    else if (code !== 0) console.error(`Browser backend exited with code ${code}`);
  });
}

const app = express();
const proxy = httpProxy.createProxyServer({ target: BACKEND_URL, ws: true, changeOrigin: false, xfwd: true });
...
<title>Authorize ChatGPT Web Agent</title><style>
:root{color-scheme:light dark;font-family:Inter,ui-sans-serif,system-ui,sans-serif}body{margin:0;min-height:100vh;display:grid;place-items:center;background:#f4f4f5;color:#18181b}.card{width:min(440px,calc(100% - 32px));background:#fff;border:1px solid #e4e4e7;border-radius:18px;padding:28px;box-shadow:0 14px 45px rgba(0,0,0,.08)}h1{font-size:22px;margin:0 0 8px}p{color:#52525b;line-height:1.55;margin:0 0 22px}label{display:block;font-size:13px;font-weight:650;margin-bottom:7px}input{width:100%;box-sizing:border-box;padding:12px 13px;border:1px solid #d4d4d8;border-radius:10px;font:inherit}button{width:100%;margin-top:14px;padding:12px 16px;border:0;border-radius:10px;background:#18181b;color:#fff;font:inherit;font-weight:700;cursor:pointer}.error{margin-bottom:14px;padding:10px 12px;border-radius:9px;background:#fee2e2;color:#991b1b;font-size:13px}.note{margin-top:16px;color:#71717a;font-size:12px}@media(prefers-color-scheme:dark){body{background:#09090b;color:#fafafa}.card{background:#18181b;border-color:#3f3f46}p,.note{color:#a1a1aa}input{background:#09090b;color:#fafafa;border-color:#52525b}button{background:#fafafa;color:#18181b}}</style></head>
<body><main class="card"><h1>Authorize ChatGPT Web Agent</h1><p>Allow your Custom GPT to control your approved Chrome session and interact directly with websites.</p>${error}<form method="post" action="/oauth/authorize">${hidden}<label for="login_password">Web Agent access password</label><input id="login_password" name="login_password" type="password" autocomplete="current-password" required autofocus><button type="submit">Authorize</button></form><div class="note">This is your Web Agent password, not your ChatGPT password.</div></main></body></html>`;
}
...
app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'chatgpt-web-agent-oauth-gateway', version: '0.4.0', auth: 'oauth2', mode: 'browser-agent' });
});

app.get('/.well-known/oauth-authorization-server', (_req, res) => res.json(getOAuthMetadata(PUBLIC_BASE_URL)));
...
app.post('/oauth/authorize', express.urlencoded({ extended: false, limit: '32kb' }), (req, res) => {
  let request;
  try { request = validateAuthorizationRequest(req.body || {}); }
  catch (error) { return res.status(400).type('html').send(`<h1>Authorization request rejected</h1><p>${htmlEscape(error.message)}</p>`); }
...
app.post('/oauth/token', express.urlencoded({ extended: false, limit: '32kb' }), (req, res) => {
  ...
  try {
    ...
    if (grantType === 'authorization_code') {
      return res.json(exchangeAuthorizationCode({ code: req.body?.code, redirectUri: req.body?.redirect_uri, codeVerifier: req.body?.code_verifier }));
    }
    if (grantType === 'refresh_token') return res.json(exchangeRefreshToken(req.body?.refresh_token));
...
server.listen(PUBLIC_PORT, '0.0.0.0', () => {
  ...
  console.log(`Browser backend: ${BACKEND_URL}`);
});
